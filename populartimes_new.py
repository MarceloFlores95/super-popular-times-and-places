...
single_place_info = False

### Nombre del archivo y carpeta de salida ###
file = "PopularTimesLuztopía" # De preferencia el nombre del lugar
carpeta = "PopularTimesLuztopía" # Carpeta donde se guardaran todos los json obtenidos.
...
# Requiere
radius = 2000 # Metros
loc = (20.59288318381712, -100.40927576975132) ### Latitud y longitud del lugar a examinar. ###
# Optional
keyword = ''
rankby = ''
single_place_id = [] # Example 'ChIJ4_j1to5tVo8RQBjYcPA_oXY'
...
types = ["atm", "bakery" ,"bank" ,"bar" ,"clothing_store" ,"department_store","drugstore" ,"grocery_or_supermarket", "hospital","park" ,"parking","pharmacy" ,"restaurant" ,"shopping_mall" ,"store" ,"supermarket"]
### Ejemplos ###
...
if (single_coord):
    ...
    df_merged.to_csv( './'+carpeta+'/'+file+"-merged.csv", index=False)
...
if (single_place_info):
    column_names = ["lat", "lon", "id","nombre", "direccion", "tipo", "rating"]
    #csv_places = pd.read_csv('./'+carpeta+'/'+file+"-merged.csv", names=column_names)
    csv_places = pd.read_csv('./'+carpeta+ 'single_file'+'/'+'5km_CheckData_.csv', names=column_names)
    places_id_list= csv_places.id.tolist()
    places_id_list.pop(0)
    single_place_id = places_id_list
    for spi in single_place_id:
        url = "https://maps.googleapis.com/maps/api/place/details/json?placeid={PLACE_ID}&key={APIKEY}".format(PLACE_ID = spi, APIKEY = APIKEY)
        print(url)
        response = requests.get(url)
        res = json.loads(response.text)
        if 'formatted_phone_number' not in res['result']:
            res["result"]["formatted_phone_number"] = "None"
        
        if 'user_ratings_total' not in res['result']:
            res["result"]["user_ratings_total"] = "None"
        ...
        if not os.path.exists(carpeta +'single_file'):
            os.mkdir(carpeta+'single_file')

        with open('./'+carpeta+'single_file/'+file+ spi+".csv", 'a') as c:
            writer = csv.writer(c)
    ...
    path = './'+carpeta+'single_file/'
    all_files = glob.glob(os.path.join(path, "*.csv"))
    df_from_each_file = (pd.read_csv(f, sep=',') for f in all_files)
    df_merged = pd.concat(df_from_each_file, ignore_index=True)
    df_merged.drop_duplicates(subset="id", keep = 'first', inplace = True) 
    df_merged.to_csv( './'+carpeta+'single_file/'+file+"-merged.csv", index=False) 
